...
public:
     ...
     */
    RF24(PinName mosi, PinName miso, PinName clk, PinName _cepin, PinName _cspin, uint32_t _spispeed = RF24_SPI_SPEED);
     ...
     * @endcode
     * @note If used from within an interrupt, the interrupt should be disabled until completion, and sei(); called to enable Kernel::get_ms_count().
     * @see txStandBy()
     * @see write()
     * @see writeFast()
     *
 ...
 * | compatibility.h    | Provides standardized timing (Kernel::get_ms_count(), delay()) methods                     |
