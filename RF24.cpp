...
{
    beginTransaction();
    status = static_cast<uint8_t>(_SPI.write(R_REGISTER | reg));
    while (len--) {
        *buf++ = static_cast<uint8_t>(_SPI.write(0xFF));
    }
    endTransaction();
}
...
uint8_t RF24::read_register(uint8_t reg)
{
    ...
    *ptx++ = RF24_NOP; // Dummy operation, just for reading
    ...
    beginTransaction();
    status = _SPI.write(R_REGISTER | reg);
...
void RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
    ...
    *ptx++ = (W_REGISTER | (REGISTER_MASK & reg));
    ...
    beginTransaction();
    status = _SPI.write(W_REGISTER | reg);
    while (len--) {
        _SPI.write(*buf++);
    }
    endTransaction();
...
void RF24::write_register(uint8_t reg, uint8_t value, bool is_cmd_only)
{
    if (is_cmd_only) {
        if (reg != RF24_NOP) { // don't print the get_status() operation
            IF_SERIAL_DEBUG(printf("write_register(%02x)\r\n", reg));
        }
        beginTransaction();
        status = _SPI.write(W_REGISTER | reg);
        endTransaction();
...
RF24::RF24(PinName mosi, PinName miso, PinName clk, PinName _cepin, PinName _cspin, uint32_t _spi_speed)
    : ce_pin(_cepin)
    , csn_pin(_cspin)
...
static const constexpr char rf24_datarate_e_str_0[] = "= 1 MBPS";
static const constexpr char rf24_datarate_e_str_1[] = "= 2 MBPS";
static const constexpr char rf24_datarate_e_str_2[] = "= 250 KBPS";
static const constexpr char* const rf24_datarate_e_str_P[] = {
    rf24_datarate_e_str_0,
    rf24_datarate_e_str_1,
    rf24_datarate_e_str_2,
};
static const constexpr char rf24_model_e_str_0[] = "nRF24L01";
static const constexpr char rf24_model_e_str_1[] = "nRF24L01+";
static const constexpr char* const rf24_model_e_str_P[] = {
    rf24_model_e_str_0,
    rf24_model_e_str_1,
};
static const constexpr char rf24_crclength_e_str_0[] = "= Disabled";
static const constexpr char rf24_crclength_e_str_1[] = "= 8 bits";
static const constexpr char rf24_crclength_e_str_2[] = "= 16 bits";
static const constexpr char* const rf24_crclength_e_str_P[] = {
    rf24_crclength_e_str_0,
    rf24_crclength_e_str_1,
    rf24_crclength_e_str_2,
};
static const constexpr char rf24_pa_dbm_e_str_0[] = "= PA_MIN";
static const constexpr char rf24_pa_dbm_e_str_1[] = "= PA_LOW";
static const constexpr char rf24_pa_dbm_e_str_2[] = "= PA_HIGH";
static const constexpr char rf24_pa_dbm_e_str_3[] = "= PA_MAX";
static const constexpr char* const rf24_pa_dbm_e_str_P[] = {
    rf24_pa_dbm_e_str_0,
    rf24_pa_dbm_e_str_1,
    rf24_pa_dbm_e_str_2,
    rf24_pa_dbm_e_str_3,
...
static const constexpr char rf24_feature_e_str_on[] = "= Enabled";
static const constexpr char rf24_feature_e_str_allowed[] = "= Allowed";
static const constexpr char rf24_feature_e_str_open[] = " open ";
static const constexpr char rf24_feature_e_str_closed[] = "closed";
static const constexpr char* const rf24_feature_e_str_P[] = {
    rf24_crclength_e_str_0,
...
/****************************************************************************/
static const constexpr uint8_t child_pipe_enable[] = { ERX_P0, ERX_P1, ERX_P2,
...
#if defined(FAILURE_HANDLING) || defined(RF24_LINUX)
    const auto timer = Kernel::get_ms_count();
#endif // defined(FAILURE_HANDLING) || defined(RF24_LINUX)

    while (!(get_status() & (_BV(TX_DS) | _BV(MAX_RT)))) {
#if defined(FAILURE_HANDLING) || defined(RF24_LINUX)
        if (Kernel::get_ms_count() - timer > 95) {
            errNotify();
#if defined(FAILURE_HANDLING)
            return 0;
#else
...
bool RF24::writeBlocking(const void* buf, uint8_t len, uint32_t timeout)
{
    ...
    uint32_t timer = Kernel::get_ms_count(); // Get the time that the payload transmission started
...
#if defined(FAILURE_HANDLING) || defined(RF24_LINUX)
    uint32_t timer = Kernel::get_ms_count();
#endif
...
#if defined(FAILURE_HANDLING) || defined(RF24_LINUX)
    uint32_t timeout = Kernel::get_ms_count();
#endif
...
#if defined(FAILURE_HANDLING) || defined(RF24_LINUX)
        if (Kernel::get_ms_count() - timeout > 95) {
            errNotify();
#if defined(FAILURE_HANDLING)
            return 0;
#endif
...
bool RF24::txStandBy(uint32_t timeout, bool startTx)
{
    ...
    while (!(read_register(FIFO_STATUS) & _BV(TX_EMPTY))) {
        if (status & _BV(MAX_RT)) {
            ...
            ce(HIGH);
            if (Kernel::get_ms_count() - start >= timeout) {
                ce(LOW);
                flush_tx();
                return 0;
            }
...
#if defined(FAILURE_HANDLING) || defined(RF24_LINUX)
        if (Kernel::get_ms_count() - start > (timeout + 95)) {
            errNotify();
#if defined(FAILURE_HANDLING)
            return 0;
#endif
...
/****************************************************************************/
static const constexpr uint8_t child_pipe[] = { RX_ADDR_P0, RX_ADDR_P1, RX_ADDR_P2,
...
void RF24::startConstCarrier(rf24_pa_dbm_e level, uint8_t channel)
{
    ...
    setPALevel(level);
    setChannel(channel);
    IF_SERIAL_DEBUG(printf(PSTR("RF_SETUP=%02x\r\n"), read_register(RF_SETUP)));
    ce(HIGH);
    if (isPVariant()) {
        ThisThread::sleep_for(1); // datasheet says 1 ms is ok in this instance
        ce(LOW);
        reUseTX();
    }
}
